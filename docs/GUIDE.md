...
- **Sort** (Live status header): **Active first** (default - recording/live users, then state, then name), **Library size** (largest finished library per user, plus bytes written for active recordings; Active section still above Watchlist), or **Name A-Z** (saved in `localStorage`)
...
4. Run `uv run poe fix-mp4s` (or `scripts/fix-mp4s.ps1`) with `-InputDir` pointing at `to_fix/`. On NVIDIA, the default path is NVDEC + CUDA scale/pad + NVENC (frames stay on the GPU). Corrupt leftovers that change size mid-stream drop the bad frame instead of aborting. Use `-AppPipeline` to run the same salvage as the recorder (libx264, HEVC-in-FLV rewrite, MKV remux) instead of NVENC. Use `-AllowCpuFallback` only if you want software scale / libx264 after a CUDA failure.
