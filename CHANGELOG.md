...
- **Media library**: hide in-flight `*.av1temp.mp4` files (external AV1 re-encodes) the same way `*.repair.tmp.mp4` temps are already omitted
...
- **Media library**: usernames that start with `_` or contain underscores (e.g. `_creator`, `_.user.99`) are no longer rejected by the `TK_…` filename pattern, so converted MP4s can play and generate thumbnails
...
- **Media library cards**: horizontal layout so filename, username, size, and date wrap fully (no line-clamp truncation); now-playing player header unchanged

## [8.20.9] - 2026-08-05

### Fixed

- **Media library**: hide the in-flight final `.mp4` while FLV conversion is running (previously only the `*_flv.mp4` source was treated as in-progress, so a half-written convert output could appear in the library)
